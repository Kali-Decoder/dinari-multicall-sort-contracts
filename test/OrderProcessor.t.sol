...
contract OrderProcessorTest is Test {
   ...
   function setUp() public {
    ...
    userPrivateKey = uint256(vm.envUint("USER_PRIVATE_KEY"));
    user = vm.addr(userPrivateKey);
    // Sepolia deployed addresses
    address orderProcessorAddress = 0x7e1eAe0370bDC16d2D8f8C61F2B1394882f38D45;
    address paymentTokenAddress = 0x665b099132d79739462DfDe6874126AFe840F7a3;
    address tokenShareAddress = 0xD771a71E5bb303da787b4ba2ce559e39dc6eD85c;
   
    // Use deployed contracts instead of deploying new ones
    issuer = OrderProcessor(orderProcessorAddress);
    paymentToken = MockToken(paymentTokenAddress);
    token = DShare(tokenShareAddress);
    sigUtils = new SigUtils(paymentToken.DOMAIN_SEPARATOR());

    console.log("Token address: %s", address(token));
    

    // Skip the admin setup, assume it is already done
    // vm.startPrank(admin);
    ...
    function getCustomDummyOrder(bool sell, uint256 paymentQty) internal view returns (IOrderProcessor.Order memory) {
    return IOrderProcessor.Order({
        requestTimestamp: uint64(block.timestamp),
        recipient: user,
        assetToken: address(token),
        ...
        sell: sell,
        orderType: IOrderProcessor.OrderType.MARKET,
        assetTokenQuantity: sell ? paymentQty : 0,
        paymentTokenQuantity: sell ? 0 : paymentQty,
        price: 0,
        tif: IOrderProcessor.TIF.GTC
    });
}
  ...
  function testRequestBuyOrderWithPermit_TwoOrders() public {
      IOrderProcessor.Order memory order1 = getCustomDummyOrder(false, 10 ether);
    IOrderProcessor.Order memory order2 = getCustomDummyOrder(false, 20 ether);

    console.log("Maker1: %s", order1.recipient);
    console.log("Maker2: %s", order2.recipient);

    uint256 quantityIn1 = order1.paymentTokenQuantity + dummyOrderFees;
    uint256 quantityIn2 = order2.paymentTokenQuantity + dummyOrderFees;
    uint256 totalQuantityIn = quantityIn1 + quantityIn2;

    // Assume user already has tokens on the forked network
    uint256 userBalance = paymentToken.balanceOf(user);
    console.log("User balance: %s", userBalance);
    require(userBalance >= totalQuantityIn, "Insufficient user balance on forked network");

    // Build permit
   SigUtils.Permit memory permit = SigUtils.Permit({
        owner: user,
        spender: address(issuer),
        value: totalQuantityIn,
        nonce: 0,
    ...
    calls[1] = abi.encodeWithSelector(OrderProcessor.createOrderStandardFees.selector, order1);
    calls[2] = abi.encodeWithSelector(OrderProcessor.createOrderStandardFees.selector, order2);

    uint256 orderId1 = issuer.hashOrder(order1);
    uint256 orderId2 = issuer.hashOrder(order2);
    console.log(orderId1,orderId2);


    emit OrderCreated(orderId1, user, order1, dummyOrderFees);
    emit OrderCreated(orderId2, user, order2, dummyOrderFees);
    ...
    // assertEq(uint8(issuer.getOrderStatus(orderId1)), uint8(IOrderProcessor.OrderStatus.ACTIVE));
    // assertEq(uint8(issuer.getOrderStatus(orderId2)), uint8(IOrderProcessor.OrderStatus.ACTIVE));

    // assertEq(issuer.getUnfilledAmount(orderId1), order1.paymentTokenQuantity);
    // assertEq(issuer.getUnfilledAmount(orderId2), order2.paymentTokenQuantity);

    // assertEq(paymentToken.balanceOf(user), userBalanceBefore - totalQuantityIn);
    // assertEq(
    //     paymentToken.balanceOf(operator),
    //     operatorBalanceBefore + order1.paymentTokenQuantity + order2.paymentTokenQuantity
    // );
}
